...
本项目由 AI 辅助生成，代码结构、逻辑实现均由人工智能（OpenClaw / Claude）基于用户需求自动编写，开发者已尽力验证功能可用性，但不保证代码在所有场景下的正确性与稳定性。

**使用本项目即表示您同意以下条款：**

1. 本项目仅供学习与个人使用，使用者应遵守企业微信相关服务条款及当地法律法规。
2. 因使用本项目造成的任何直接或间接损失（包括但不限于账号封禁、数据丢失、业务中断等），开发者不承担任何法律责任。
3. 本项目不保证持续维护与更新，不提供任何形式的担保。
4. 使用者应自行承担使用风险，建议在测试环境中充分验证后再投入生产使用。
5. 本项目涉及企业微信后台自动化操作，请确保您拥有合法的管理员权限。
